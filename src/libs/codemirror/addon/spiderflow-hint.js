import { getGrammers } from '@/api/spider'

let grammers = []

getGrammers().then(response => {
  if (response.code === 1) {
    grammers = response.data
    for (let i = 0, len = grammers.length; i < len; i++) {
...
function searchGrammer(keyword, isClass) {
  ...
  for (let i = 1, len = list1.length; i < len; i++) {
...
export function initHint(cm) {
  cm.on('keyup', function(cm, e) {
    ...
    if ((str1 === '{' && str2 === '$')) {
      const ret = searchGrammer(null, true)
      const datas = {}
      datas.list = ret[0]
      datas.from = {}
      ...
      if (keyword && keyword[1]) {
        keyword = keyword[1]
        const ret = searchGrammer(keyword)
        const datas = {}
        datas.list = ret[0]
        datas.from = {}
        datas.from.line = cur.line
